...
- 当前 Vulkan primitive 已有 bbox 矩形第一段，使用 render-pass 内 `vkCmdClearAttachments` 绘制实心矩形并尊重后端 scissor 状态；这只是让 UI 块可见的过渡层，不替代后续 shader pipeline。
- 当前 Vulkan text/image 只是 no-op 链接占位；稳定 UI 绘制必须在阶段 5 拆完 CPU/GPU 资源后接入。
...
- primitive 第一段可显示 bbox 实心矩形；text/image 继续保持 no-op，直到 frame lifecycle 和 primitive pipeline 稳定。
